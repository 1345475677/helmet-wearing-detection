...
from models.Tiny_Face.Tiny_Face import tiny_face_model
from models.Tiny_Face.Tiny_Face import util
...
import pandas as pd
from models.Tiny_Face.Tiny_Face import tensorflow_simpleCNN2_evaluate
...
def evaluate(weight_file_path, vedio_path, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
...
def save_as_csv(mulboxes):
    ...
    print(df)
    df.to_csv("D:/service2019/test4.csv")

def vedio_cnn(mulboxes,vedio_path):
    mulboxes = tensorflow_simpleCNN2_evaluate.cnn_for_vedio(mulboxes,vedio_path)
    print(mulboxes)
    df = pd.DataFrame(columns=('frame','class','xmin', 'ymin','xmax', 'ymax','score','score_class'))
    for boxes in mulboxes:
        _df = pd.DataFrame(boxes,columns=('frame','class','xmin', 'ymin','xmax', 'ymax','score','score_class'))
        df = pd.concat([df,_df],axis=0,ignore_index=True)

    df = df.drop(['score'], axis=1)
    df = df.rename(columns={'score_class': 'score'})
    print(df)
    #df.to_csv('D:/service2019/test5.csv')
    return df
...
def main(video):
  ...
  """
 # vedio = 'D:/service2019/video/6.mp4'
  with tf.Graph().as_default():
    evaluate(weight_file_path='models\Tiny_Face/mat2tf.pkl',vedio_path=video)
  ...
  #save_as_csv(mulboxes)
  df=vedio_cnn(mulboxes,video)
  return df
