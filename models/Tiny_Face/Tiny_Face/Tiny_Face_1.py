  ...
  with tf.Session() as sess:
    ...
    for filename in filenames:
      ...
      for r in refined_bboxes:
          ...
          y1-=differ_y/1.5
          y2=z+differ_y/10
          if(r[3]>raw_img.shape[0]):r[3]=raw_img.shape[0]-10
          if(x2>raw_img.shape[1]):x2=raw_img.shape[1]-10
          if(x1<=0): x1=10
          if(y1<=0): y1=10
      ...
      df=pd.DataFrame(columns=["filename","xmin","ymin","xmax","ymax","score"])
      print(df)
      _df = pd.DataFrame(box_hel, columns=('frame', 'xmin', 'ymin', 'xmax', 'ymax', 'score'))
      df = pd.concat([df, _df], axis=0, ignore_index=True)

      df.to_csv("D:\学习\服务外包竞赛\Object-Detection_HelmetDetection-master\data/test_result_face.csv",index=False,header=False,mode="a+")
      ...
      cv_imwrite(os.path.join(output_dir, fname), raw_img)
...
def main():
  ...
  with tf.Graph().as_default():
    evaluate(weight_file_path='D:/anaconda\models/research\Tiny_Face/mat2tf.pkl', data_dir='D:\学习\服务外包竞赛\Object-Detection_HelmetDetection-master\data/test/new_test', output_dir='D:\学习\服务外包竞赛\Object-Detection_HelmetDetection-master\data/test_results')
